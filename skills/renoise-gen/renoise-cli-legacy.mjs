...
var RenoiseClient = class {
  ...
  async getUserId() {
    ...
    const account = await this.getMe();
    this.userId = account.user?.id;
    if (!this.userId) throw new Error("Account response did not include a user id");
    return this.userId;
  }
  ...
  async listTasks(params = {}) {
    const qs = new URLSearchParams();
    qs.set("user_id", await this.getUserId());
  ...
  async getTask(id) {
    const data = await this.request("GET", `/tasks/${id}`);
    const owner = await this.getUserId();
    if (data.task?.userId && data.task.userId !== owner) throw new ApiError(403, {}, "Task does not belong to this API key");
    return data;
  }
...
var MODEL_CONSTRAINTS = {
  ...
  "kling-3.0-omni": { type: "video", resolutions: ["720p", "1080p"], ratios: ["16:9", "9:16", "1:1"], maxRefImages: 7, maxRefVideos: 1 },
...
Authentication:
  renoise auth login  Preferred shared saved credential.
  RENOISE_API_KEY     Optional environment override.
...
`);
    ...
    if (e instanceof AuthError) {
      ...
      console.error("Run `renoise auth login` or check RENOISE_API_KEY.");
      process.exit(1);
    }
