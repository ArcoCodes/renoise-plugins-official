...
test("self-contained widget boots into a compact, inert review launcher without external assets", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Renoise Visual Edit" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Approve and open visual editor" })).toBeDisabled();
